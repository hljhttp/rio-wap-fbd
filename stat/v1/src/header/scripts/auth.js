...
const FORMVIEWER_FOR_MOBILE_REDIRECT_URL = 'https://localhost/formviewer/index.html';
...
function _getJWTInfo() {
    ...
    if (ADAL!=null || MSAL!=null || hello_js_support.isInitialized()) {
        try {
            var encodedIdToken = ADAL!=null ? ADAL._getItem(ADAL.CONSTANTS.STORAGE.IDTOKEN) : MSAL!=null ? MSAL._cacheStorage.getItem("msal.idtoken") : hello_js_support.getSession().id_token;
...
function prepareRightUrlAndResourceIdForGraphCall(url, resourceId) {
    ...
    if (appConfiguration.useGraphProxyApi===true) {
        if (!appConfiguration.home || !appConfiguration.actionGraphProxy) {
            ...
            ret.resourceId = ADAL!=null ? ADAL.config.clientId : MSAL!=null ? MSAL.clientId : hello_js_support.getClientId();
        }
    }
    return ret;
}
...
function getUserThumbnailPhoto() {
    var tenantId = ADAL==null ? ((MSAL==null ? (!hello_js_support.isInitialized() ? "" : TogFormViewer.Utils._getTenantIdFromIdentityProviderURL(TogFormViewer.Utils._extractIdToken(hello_js_support.getSession().id_token).iss)) : (isAAD() ? MSAL.getUser().idToken.tid : TogFormViewer.Utils._getTenantIdFromIdentityProviderURL(MSAL.getUser().identityProvider)))) : ADAL.config.tenant;
...
function sendfeedback(url, payload, additionalConfiguration) {
    executeAjaxRequestWithAdalLogic(ADAL!=null ? ADAL.config.clientId : MSAL!=null ? MSAL.clientId : hello_js_support.getClientId(), postfeedbackformadal, url, payload, additionalConfiguration);
}
