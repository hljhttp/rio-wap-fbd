...
function loadScriptFormBuilder(){

    urlQuery = parse_query_string(window.location.search.substring(1));

    for(key in builderObj){
        
        if(urlQuery.hasOwnProperty(key)){
            loadScript(urlQuery[key]);
        }else{
            loadScript(builderObj[key]);
        }
    }
}

function destroyFormViewer () {
    resetAppConfiguration();
    resetFormData();
    appURLQueryParameters = null;
...
function resetAppConfiguration()
{
    appConfiguration =
    {
        ...
        actionGraphProxy: "",
        actionLoading: "",
        actionLoadingBeforeScript: "",
        actionLoadingSuccessScript: "",
        actionLoadingFailureScript: "",
        actionLoaded: "",
        actionFocus: "",
    ...
    appConfiguration.onlinemode = typeof ADAL!== 'undefined' && ADAL!=null || typeof MSAL!== 'undefined' && MSAL!=null || typeof hello_js_support!=='undefined' && hello_js_support!=null && hello_js_support.isInitialized();
...
function initAfterADALSetup()
{
    ...
    if (ADAL!=null || MSAL!=null || hello_js_support.isInitialized()) {
        ...
        if (signeduser) {
            ...
            currentUser.name = ADAL!=null ? signeduser.profile.name : signeduser.idToken.name;
            if (MSAL!=null) {
                try {
...
function _getopenidconfig(idt) {
    if (idt) {
        console.log('getting openid-configuration...');
        var settings = {
            "crossDomain": true,
            "url": idt.iss+"/.well-known/openid-configuration"+(isAAD() ? "" : "?p="+(idt.tfp ? idt.tfp : idt.acr)),
            "timeout":30000,
            "method": "GET",        
        };
 ...
 */
function loadScript(url, callback, errorHandler, idElement)
{
...
function loadScriptWithGetParams(url, callback, errorHandler)
{
    var scriptElement = document.createElement("script");
    scriptElement.type = "text/javascript";
...
function setupHeaderConfiguration()
{
    ...
    resolveStringOrBooleanParameter(false,"Appl-Home","home",null,headerObj,null,true,appConfiguration.home);     
    ...
    // setup loading action before script
    resolveStringOrBooleanParameter(false,"action loading before script","actionLoadingBeforeScript",formObj,headerObj,null,true,appConfiguration.actionLoadingBeforeScript); 
    
    // setup loading action success script
    resolveStringOrBooleanParameter(false,"action loading success script","actionLoadingSuccessScript",formObj,headerObj,null,true,appConfiguration.actionLoadingSuccessScript); 
    
    // setup loading action failure script
    resolveStringOrBooleanParameter(false,"action loading failure script","actionLoadingFailureScript",formObj,headerObj,null,true,appConfiguration.actionLoadingFailureScript); 
	...
	// Override the default alternate map initialization procedure
	resolveStringOrBooleanParameter(false, "map before render callback","mapBeforeRenderCallback",formObj,headerObj,null,true,appConfiguration.mapBeforeRenderCallback);
...
{    
    ...
    if (appConfiguration.actionLoadingBeforeScript) {
        executeScript(event.controlId+"-actionBeforeScript",appConfiguration.actionLoadingBeforeScript,event,true);
    }
    ...
    if (typeof ADAL!== 'undefined' && ADAL || typeof MSAL!== 'undefined' && MSAL || typeof hello_js_support!=='undefined' && hello_js_support && hello_js_support.isInitialized()) {
        executeAjaxRequestWithAdalLogic(ADAL!=null ? ADAL.config.clientId : MSAL!=null ? MSAL.clientId : hello_js_support.getClientId(), executeAjaxRequest, url, payload, {"callbackCount":cnt,"event":event},onsuccess_loading,onfailure_loading,onfailure_loading);
    } else {
...
function onsuccess_loading(token,url,formdata,additionalConfiguration,data,textStatus,request) {
    ...
    handleServerResponseForLoadingAndOtherActions(url,additionalConfiguration,data, appConfiguration.actionLoadingSuccessScript);
}
...
function handleServerResponseForLoadingAndOtherActions(url,additionalConfiguration,data,actionSuccessScript) {   
   ...
   if (appInfoObjFromServer!=null) {
       ...
       if (resolvedPropertiesObjFromServer.onlinemode!=null && !resolvedPropertiesObjFromServer.onlinemode) {
          ADAL = null;
          MSAL = null;
          if (hello_js_support) hello_js_support.setOffline();
       }
...
function onfailure_loading(token,url,formdata,additionalConfiguration,err,textStatus,errorThrown) {
   if (appConfiguration.actionLoadingFailureScript) {
      executeScript(additionalConfiguration.event.controlId+"-actionFailureScript",appConfiguration.actionLoadingFailureScript,additionalConfiguration.event,true);
   } else {
...
var TogFormViewer =
{
    init: function()
    {
        ...
        TogFormViewer.menuPosition = "top";
        TogFormViewer.testFlag = false;
        TogFormViewer.debugFlag = false;
        TogFormViewer.activeMenuKey = "";
        TogFormViewer.openidConfiguration = null;
        TogFormViewer.dirty = false;
        TogFormViewer.additionalAppInfoData = {};
        TogFormViewer.correlationId = TogFormViewer.uuidv4();
        TogFormViewer.lastCalculationTimestamp = null;
        TogFormViewer.lastCalculationResultTimestamp = null;
    ...
    setProperty: function(propName, propValue)
    {
        ...
        {
            appConfiguration.mapGridSize = Number(propValue);
            MapPlugIn.changeMapGridSize(Number(propValue));	
        }
		...
		else if (_checkPropertyValue(propName, propValue, "mapBeforeRenderCallback", "string"))
        {
            appConfiguration.mapBeforeRenderCallback = propValue;
			MapPlugIn.reloadMap();
        }
    ...
    getAppInfo: function(event,sendForm)
    {
        var appInfo = {
            ...
            "runtimeProperties" : {
                "tenantId" : (typeof ADAL=== 'undefined' || ADAL==null ? ((typeof MSAL=== 'undefined' || MSAL==null ? (typeof hello_js_support==='undefined' || hello_js_support==null || !hello_js_support.isInitialized() ? "" : TogFormViewer.Utils._getTenantIdFromIdentityProviderURL(TogFormViewer.Utils._extractIdToken(hello_js_support.getSession().id_token).iss)) : (isAAD() ? MSAL.getUser().idToken.tid : TogFormViewer.Utils._getTenantIdFromIdentityProviderURL(MSAL.getUser().identityProvider)))) : ADAL.config.tenant),
                "appRegAppId" : (typeof ADAL=== 'undefined' || ADAL==null ? ((typeof MSAL=== 'undefined' || MSAL==null ? (typeof hello_js_support==='undefined' || hello_js_support==null || !hello_js_support.isInitialized() ? "" : hello_js_support.getClientId()) : MSAL.clientId)) : ADAL.config.clientId),
            ...
            "auth" : {
                "IDToken" : _getJWTInfo(),
                "IDTokenEncoded" : (typeof ADAL=== 'undefined' || ADAL==null ? ((typeof MSAL=== 'undefined' || MSAL==null ? (typeof hello_js_support==='undefined' || hello_js_support==null || !hello_js_support.isInitialized() ? null:hello_js_support.getSession().id_token) : MSAL._cacheStorage.getItem("msal.idtoken"))) : ADAL._getItem(ADAL.CONSTANTS.STORAGE.IDTOKEN)),
...
function _performCalculationRemotely()
{
    ...
    if (typeof ADAL!== 'undefined' && ADAL || typeof MSAL!== 'undefined' && MSAL || typeof hello_js_support!=='undefined' && hello_js_support && hello_js_support.isInitialized()) {
        executeAjaxRequestWithAdalLogic(ADAL!=null ? ADAL.config.clientId : MSAL!=null ? MSAL.clientId : hello_js_support.getClientId(), executeAjaxRequest, url, payload, additionalConfig,onsuccess_calc,onfailure_calc,onfailure_calc);
    } else {
...
function getScript(url) {
    ...
    for (var i = scripts.length; i--;) {
        if (scripts[i].attributes.src && scripts[i].attributes.src.value === url) return scripts[i];
    }
    return null;
}
...
function executeAjaxRequest(token,url,formdata,additionalConfiguration,onsuccess,onfailure) {
   var httpMethod = additionalConfiguration ? additionalConfiguration.actionHttpMethod : null;
   var timeout = additionalConfiguration ? additionalConfiguration.actionTimeout : 60000;
   if (httpMethod) {
